...
def format_rk(r):
    ...
    name_s   = r["name"][:42] + ("…" if len(r["name"]) > 42 else "")
    lines = [
        f"{status} <b>{name_s}</b>",
        f"   💰 ${r['spend']:.2f} | 🛒 {r['purchases']} | CPP {cpp_str} | CR {cr_str} | CPM ${r['cpm']:.2f}",
        f"   🎬 Hook {hook_str} | CTR {r['ctr']:.2f}% | Freq {r['frequency']:.1f}",
    ]
    if issues:    lines.append(f"   ⚠️ {' · '.join(issues)}")
    if positives: lines.append(f"   ✅ {' · '.join(positives)}")
    return "\n".join(lines)
...
def format_account_block(rows, acc_name):
    ...
    lines += [
        f"\n{'─'*32}",
        f"📊 <b>TOTAL {acc_name}:</b> 💰${total_spend:.2f} | 🛒{total_pur} | CPP {cpp_str} | CR {total_cr}% | CPM ${avg_cpm}",
    ]
